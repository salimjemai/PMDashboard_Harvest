    ...
    public void addNewChangesRecord(Statement statement,Statement statement1 ,ArrayList<Person> personArrayList,Create_Insert_Queries create_insert_queries) throws SQLException
    {
        ...
        try {
            ...
            for (int i = 0; i < personArrayList.size(); i++)
            {
                ...
                resultSet = statement.executeQuery("Select * from changes where id_users ="+personArrayList.get( i ).getId()+ " and (role = '"
                                                            + personArrayList.get( i ).getRoles() + "' and status ="
                                                            + personArrayList.get( i ).getIs_active() + " and hourly_rate = '"
                                                            + personArrayList.get( i ).getDefault_hourly_rate() + "')" );
...
public boolean noChanges(Statement statement,ArrayList<Person> personArrayList) throws SQLException
{
    boolean noChanges = false;
    try {
        for (int i = 0; i < personArrayList.size(); i++)
        {
            ResultSet resultSet1 = statement.executeQuery( "select * from changes where id_users = "
                                                                    + personArrayList.get( i ).getId() + " and (role = '"
                                                                    + personArrayList.get( i ).getRoles() + "' and status ="
                                                                    + personArrayList.get( i ).getIs_active() + " and hourly_rate = '"
                                                                    + personArrayList.get( i ).getDefault_hourly_rate() + "')" );
